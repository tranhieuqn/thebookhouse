...
public class UserController {
    ...
    public User deactive(@PathVariable int userId) {
        User user = userService.findOne(userId);
        if (user == null) {
            throw new EntityException(
                    "User not found. Please check everything again.", HttpStatus.BAD_REQUEST);
        }
        return userService.deactive(user);
    }

    @PutMapping("/updateAddress/{userId}")
    public User updateAddress(@Valid @RequestBody Address address, @PathVariable int userId,
        ...
        return userService.updateAddress(user, address);
    }
}
